...
@Component({
  ...
  styleUrls: ['./anexo-tres.component.scss']
})

export class AnexoTresComponent implements OnInit {
  ...
  constructor(
    ...
    this.documentosForm = this.fb.group({
      tipo_documento_id: [3, Validators.required],
  ...
  onSubmit() {
    if (this.documentosForm.valid) {
      ...
      this.documentosForm.patchValue({ tipo_documento_id: 3 }); // id 3 inf. financera
      ...
      this._anexosService.PostAnexos3(formData, this.token!).subscribe({
        next: ({ message }) => {
